...
struct AccountEmailLinkingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 48) {
                    VStack(spacing: 12) {
                        Text(isLinkSent ? "メールを確認してください" : "メールアドレスで連携")
                            .font(.system(size: 24, weight: .bold))
                            .foregroundColor(.dsForeground)
                        
                        Text(isLinkSent ? "\(email) 宛に連携リンクを送信しました。メール内のリンクをタップしてアプリに戻ると連携が完了します。" : "入力したメールアドレスに連携用のリンクを送信します。")
                            .font(.subheadline)
